...
kubectl apply -f nginx_controller/namespace.yaml 

echo -e "${RED}Setting up Role-Based Access Control (RBAC) ${NC}"

kubectl apply -f nginx_controller/rbac.yaml 

echo -e "${RED}Setting up ConfigMap ${NC}"

kubectl apply -f nginx_controller/config_map.yaml 
...
kubectl apply -f nginx_controller/default_http_backend.yaml

echo -e "${RED}Setting up nginx-ingress-controller ${NC}"

kubectl apply -f nginx-controller/nginx_ingress_controller.yaml

echo -e "${RED}Setting up nginx-ingress-rules for REST and for gRPC ${NC}"

kubectl apply -f nginx-controller/nginx_ingress_REST_rules.yaml

kubectl apply -f nginx-controller/nginx_ingress_gRPC_rules.yaml
